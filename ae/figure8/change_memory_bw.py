...
def test_memory_bandwidth(memory_bandwidth, lock):
    ...
    print(f"{memory_bandwidth}, {compute_area_mm2}, {io_area_mm2}, {compute_area_mm2 + io_area_mm2}")
    ...
    print(f"{memory_bandwidth}, {init_latency_simulated}, {auto_regression_latency_simulated}")
    with lock:
...
lock = Lock()
processes = [Process(target=test_memory_bandwidth, args=(i, lock)) for i in [1, 2, 3, 4, 5, 6, 7, 8]]
