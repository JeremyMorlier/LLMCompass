...
def test_SRAM_KB(SRAM_KB, lock):
    ...
    print(f"{SRAM_KB}, {compute_area_mm2}, {io_area_mm2}, {compute_area_mm2 + io_area_mm2}")
    ...
    with lock:
        with open("ae/figure9/l1_cache_results_init.csv", "a") as f:
            f.write(
                f"{SRAM_KB}, {compute_area_mm2 + io_area_mm2}, {init_latency_simulated}, {model_init.simluate_log}\n"
            )
        with open("ae/figure9/l1_cache_results_ar.csv", "a") as f:
            f.write(
...
lock = Lock()
processes = [Process(target=test_SRAM_KB, args=(i, lock)) for i in [64, 128, 192, 256, 512, 1024]]
