...
softmax_TPUv3_sim = pd.read_csv("softmax_TPUv3_sim.csv", header=None, names=["M", "N", "throughput"])
softmax_TPUv3_sim.set_index(["M", "N"], inplace=True)
softmax_TPUv3_roofline = pd.read_csv("softmax_TPUv3_roofline.csv", header=None, names=["M", "N", "throughput"])
softmax_TPUv3_roofline.set_index(["M", "N"], inplace=True)
softmax_A100 = pd.read_csv("real_hardware/softmax_A100.csv", header=None, names=["M", "N", "throughput"])
softmax_A100.set_index(["M", "N"], inplace=True)
softmax_A100_sim = pd.read_csv("softmax_A100_sim.csv", header=None, names=["M", "N", "throughput"])
softmax_A100_sim.set_index(["M", "N"], inplace=True)
softmax_A100_roofline = pd.read_csv("softmax_A100_roofline.csv", header=None, names=["M", "N", "throughput"])
softmax_A100_roofline.set_index(["M", "N"], inplace=True)
softmax_MI210 = pd.read_csv("real_hardware/softmax_MI210.csv", header=None, names=["M", "N", "throughput"])
softmax_MI210.set_index(["M", "N"], inplace=True)
softmax_MI210_sim = pd.read_csv("softmax_MI210_sim.csv", header=None, names=["M", "N", "throughput"])
softmax_MI210_sim.set_index(["M", "N"], inplace=True)
softmax_MI210_roofline = pd.read_csv("softmax_MI210_roofline.csv", header=None, names=["M", "N", "throughput"])
...
for N in range(6, 16):
    ...
    throughput_TPU_sim_list.append(softmax_TPUv3_sim.loc[(M, N), "throughput"].values[0])
    throughput_TPU_roofline_list.append(softmax_TPUv3_roofline.loc[(M, N), "throughput"].values[0])
    ...
    throughput_GPU_roofline_list.append(softmax_A100_roofline.loc[(M, N), "throughput"].values[0])
    throughput_AMD_list.append(softmax_MI210.loc[(M, N), "throughput"].values[0])
    throughput_AMD_sim_list.append(softmax_MI210_sim.loc[(M, N), "throughput"].values[0])
    throughput_AMD_roofline_list.append(softmax_MI210_roofline.loc[(M, N), "throughput"].values[0])
...
plt.plot(N_list, throughput_AMD_list, marker="o", label="Real AMD MI210", color=color_AMD[1])
plt.plot(
    N_list,
    throughput_AMD_sim_list,
    marker="x",
