...
gelu_TPUv3_sim = pd.read_csv("gelu_TPUv3_sim.csv", header=None, names=["M", "throughput"])
gelu_TPUv3_roofline = pd.read_csv("gelu_TPUv3_roofline.csv", header=None, names=["M", "throughput"])
...
gelu_A100_roofline = pd.read_csv("gelu_A100_roofline.csv", header=None, names=["M", "throughput"])
gelu_MI210 = pd.read_csv("real_hardware/gelu_MI210.csv", header=None, names=["M", "throughput"])
gelu_MI210_sim = pd.read_csv("gelu_MI210_sim.csv", header=None, names=["M", "throughput"])
gelu_MI210_roofline = pd.read_csv("gelu_MI210_roofline.csv", header=None, names=["M", "throughput"])
...
for M in range(10, 30):
    ...
    throughput_TPU_sim_list.append(gelu_TPUv3_sim[gelu_TPUv3_sim["M"] == M]["throughput"].iloc[0])
    throughput_TPU_roofline_list.append(gelu_TPUv3_roofline[gelu_TPUv3_roofline["M"] == M]["throughput"].iloc[0])
    throughput_GPU_list.append(gelu_A100[gelu_A100["M"] == M]["throughput"].iloc[0])
    throughput_GPU_sim_list.append(gelu_A100_sim[gelu_A100_sim["M"] == M]["throughput"].iloc[0])
    throughput_GPU_roofline_list.append(gelu_A100_roofline[gelu_A100_roofline["M"] == M]["throughput"].iloc[0])
    throughput_AMD_list.append(gelu_MI210[gelu_MI210["M"] == M]["throughput"].iloc[0])
    throughput_AMD_sim_list.append(gelu_MI210_sim[gelu_MI210_sim["M"] == M]["throughput"].iloc[0])
    throughput_AMD_roofline_list.append(gelu_MI210_roofline[gelu_MI210_roofline["M"] == M]["throughput"].iloc[0])
...
plt.savefig("figure5g.pdf", dpi=300, bbox_inches="tight", pad_inches=0.01)
