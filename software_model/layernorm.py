...
class LayerNorm(Operator):
    ...
    class Mapping:
        ...
        def display(self):
            print("-" * 20)
            print(f"l2_tile_M: {self.l2_tile_M}, l1_tile_M: {self.l1_tile_M}, l1_tile_N: {self.l1_tile_N}")
    ...
    def compile_and_simulate(self, pcb_module: Device, compile_mode: str):
        ...
        l2_tile_N = N
        l2_tile_M = pcb_module.compute_module.l2_size // (l2_tile_N * data_type.word_size) // 2
        l2_tile_M = min(l2_tile_M, M)
        if compile_mode == "heuristic-GPU" or compile_mode == "heuristic-our-throughput":
            # if N <= 1024:
            l1_tile_N = N
            l1_tile_M = pcb_module.compute_module.core.SRAM_size // (l1_tile_N * data_type.word_size) // 2
    ...
    class L2TileSimulator:
        ...
        def simulate_l2_tile_compute_cycle_count(
            ...
            l1_tile_cycle_count = l1_tile.read_cycle_count * 3 + l1_tile.write_cycle_count + l1_tile.compute_cycle_count
    ...
    class L1TileSimulator:
        def __init__(
            ...
            self.reduction_cycle_count = (
                M * N / pcb_module.compute_module.core.vector_unit.total_vector_flops_per_cycle
                + M
                * N
                * data_type.word_size
                * 2
        ...
        def simulate_l1_tile_compute_cycle_count(
            ...
            M_per_vector_count = ceil(M / pcb_module.compute_module.core.vector_unit.vector_count)
            N_per_vector_count = N
            M_per_vector_lane = M_per_vector_count
            N_per_vector_lane = ceil(N_per_vector_count / pcb_module.compute_module.core.vector_unit.vector_width)

            # each lane computes it own mean
            total_cycle_count = ceil(
                N_per_vector_lane * M_per_vector_lane / pcb_module.compute_module.core.vector_unit.flops_per_cycle
            )
            # the whole vector reduce to one mean
            total_cycle_count += log2(pcb_module.compute_module.core.vector_unit.vector_width)
            # each lane computes it own variance
            total_cycle_count += (
                ceil(N_per_vector_lane * M_per_vector_lane / pcb_module.compute_module.core.vector_unit.flops_per_cycle)
                * 2
            )
