...
class AllReduceMultiPCB(CommunicationPrimitive):
    ...
    def simulate(self, interconnect_module: InterConnectModule) -> None:
        ...
        link_bandwidth_per_direction = interconnect_module.link_module.bandwidth_per_direction
        link_bandwidth_both_direction = interconnect_module.link_module.bandwidth_both_direction
        link_latency = interconnect_module.link_module.latency
        _ = interconnect_module.link_module.flit_size
        ...
        if interconnect_module.topology == TopologyType.FC:
            edge_bandwidth_per_direction = link_bandwidth_per_direction * link_count_per_device / (device_count - 1)
            edge_bandwidth_both_direction = link_bandwidth_both_direction * link_count_per_device / (device_count - 1)
            edge_latency = link_latency
            data_size_per_device = data_size / device_count
            effective_data_size_per_device = (
                header_size + ceil(data_size_per_device / max_payload_size) * header_size + data_size_per_device
            )
            # stage 1: ring reduce
            latency = (edge_latency + effective_data_size_per_device / edge_bandwidth_both_direction) * (
                device_count - 1
            )
            # stage 2: broadcast
            latency += effective_data_size_per_device / edge_bandwidth_per_direction
            latency += data_size / interconnect_module.internal_link_bandwidth_per_direction
            ...
            latency = (edge_latency + per_transmission_latency) * ((device_count - 1) * 2)
            latency += data_size / interconnect_module.internal_link_bandwidth_per_direction
            self.latency = latency
        else:
